...
class TychoUtilities {
  ...
  static List<String> addOsgiClasspathElements(MavenProject project) {
    ...
    List<Object> classpath =
        (List<Object>) getContextValue(project, TychoConstants_CTX_ECLIPSE_PLUGIN_CLASSPATH);
    if (classpath == null || classpath.isEmpty()) return Collections.emptyList();

    List<String> osgiClasspath = new ArrayList<>();
    for (Object classpathEntry : classpath) {
      for (File file : getLocations(classpathEntry)) osgiClasspath.add(file.getAbsolutePath());
    }
