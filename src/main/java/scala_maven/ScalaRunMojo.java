...
public class ScalaRunMojo extends ScalaMojoSupport {
    ...
    /**
     * Additional parameter to use to call the main class Using this parameter only
     * from command line ("-DaddArgs=arg1|arg2|arg3|..."), not from pom.xml.
     */
     ...
     * way to call an application) launchers could be define by :
     *
     * <pre>
     *   &lt;launchers&gt;
     *     &lt;launcher&gt;
     ...
     *
     * Higher priority to launcher parameter) Using this parameter only from command
     * line (-DmainClass=...), not from pom.xml.
     *
     */
    ...
    protected void doExecute() throws Exception {
        ...
        if (StringUtils.isNotEmpty(mainClass)) {
            jcmd = new JavaMainCallerByFork(this, mainClass, MainHelper.toMultiPath(project.getTestClasspathElements()),
                jvmArgs, args, forceUseArgFile, toolchain);
            ...
            if (StringUtils.isNotEmpty(launcher)) {
                for (int i = 0; (i < launchers.length) && (jcmd == null); i++) {
                    if (launcher.equals(launchers[i].id)) {
                        ...
                        jcmd = new JavaMainCallerByFork(this, launchers[i].mainClass,
                            MainHelper.toMultiPath(project.getTestClasspathElements()), launchers[i].jvmArgs,
                            launchers[i].args, forceUseArgFile, toolchain);
                    }
                }
            } else {
                getLog().info("launcher '" + launchers[0].id + "' selected => " + launchers[0].mainClass);
                jcmd = new JavaMainCallerByFork(this, launchers[0].mainClass,
                    MainHelper.toMultiPath(project.getTestClasspathElements()), launchers[0].jvmArgs, launchers[0].args,
                    forceUseArgFile, toolchain);
            }
        }
        if (jcmd != null) {
