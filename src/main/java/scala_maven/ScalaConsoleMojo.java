...
public class ScalaConsoleMojo extends ScalaMojoSupport {
    ...
    /**
     * Constant {@link String} for "jline". Used for the artifact id, and usually
     * the group id, for the JLine library needed by the Scala Console.
     */
    private static final String JLINE = "jline";

    /**
     * Constant {@link String} for "org.scala-lang". In this class it is used for
     * the forked JLine group id.
     */
    ...
    /**
     * Path of the javaRebel jar. If this option is set then the console run with
     * <a href="http://www.zeroturnaround.com/javarebel/">javarebel</a> enabled.
     *
     */
     ...
     * <p>
     * If we are unable to find it or {@link #javaRebelPath} is {@code null}, then
     * nothing is done to the given {@link JavaMainCaller}.
     *
     * @param jcmd
     *            the {@link JavaMainCaller} to which to add the JRebel settings.
     */
     ...
     *
     * @param scalaVersion
     *            the {@link VersionNumber} for the Scala Compiler/Library we are
     *            using.
     *
     * @return A {@link Set} of {@link String} defining classpath values to provide
     *         to the Scala console.
     *
     * @throws {@link
     *             Exception} for many reasons, mostly relating to ad-hoc dependency
     *             resolution.
     */
     ...
     * <ul>
     * <li>Scala Compiler, as defined by
     * {@link ScalaMojoSupport#addCompilerToClasspath}.
     * <li>Library Classpath, as defined by
     * {@link ScalaMojoSupport#addLibraryToClasspath}.
    ...
    /**
     * Construct the Classpath for any additional dependencies that are needed to
     * run the Scala console.
     * <p>
     * This should include the following entities.
     * <ul>
     ...
     * dependency synchronized with the Scala REPL, which is likely to cause binary
     * compatibility errors. If, for some reason, we are unable to resolve this
     * dependency using the Compiler's dependency tree, we fallback to a set of
     * hard-coded defaults that will <em>usually</em> work, but will not always
     * work.
     * <p>
     * If the dynamic approach to finding the JLine dependency proves stable, we may
     * drop the fallback in the future.
     *
     * @param scalaVersion
     *            the version of the Scala Compiler/Library we are using for this
     *            execution.
     * @param defaultFallback
     *            returned if we are unable to resolve JLine against the Scala
     *            Compiler's dependency tree.
     *
     * @return an {@link Artifact} to provide to the runtime of the Scala console
     *         conforming the JLine.
     */
     ...
     * <p>
     * Since different versions of the Scala Compiler were using different artifacts
     * for JLine, things are a bit tricky here. Any {@link Artifact} to have an
     * artifact id equal to {@code "jline"} and a group id equal to <em>either</em>
     * {@code "jline"} or {@code "org.scala-lang"} will yield {@code true}. The
     * latter group id corresponds to a fork of JLine that is not being used
     * anymore.
     *
     * @param artifact
     *            the {@link Artifact} to check to see if it is viable JLine
     *            candidate.
     */
    ...
    /**
     * Hard coded fallback values for JLine. This used to be the only way we resolve
     * JLine, but required manually upkeep to avoid binary comparability errors.
     * <p>
     ...
     * <li>After Scala 2.9.0 before Scala 2.11.0,
     * org.scala-lang:jline:SCALA-VERSION:jar</li>
     ...
     *            execution.
     *
     * @return a fallback {@link Artifact} which we <em>hope</em> will work with the
     *         Scala REPL.
     */
    private Artifact fallbackJLine(final VersionNumber scalaVersion) {
        ...
        if (scala2_12_0M4.compareTo(scalaVersion) <= 0) {
            return super.factory.createArtifact(ScalaConsoleMojo.JLINE, ScalaConsoleMojo.JLINE, "2.14.1", "",
                ScalaMojoSupport.JAR);
        } else if (scala2_11_0.compareTo(scalaVersion) <= 0) {
            return super.factory.createArtifact(ScalaConsoleMojo.JLINE, ScalaConsoleMojo.JLINE, "2.12", "",
                ScalaMojoSupport.JAR);
        } else if (scala2_9_0.compareTo(scalaVersion) <= 0) {
            return super.factory.createArtifact(ScalaConsoleMojo.SCALA_ORG_GROUP, ScalaConsoleMojo.JLINE,
                scalaVersion.toString(), "", ScalaMojoSupport.JAR);
        } else {
            return super.factory.createArtifact(ScalaConsoleMojo.JLINE, ScalaConsoleMojo.JLINE, "0.9.94", "",
                ScalaMojoSupport.JAR);
        }
    }
}
