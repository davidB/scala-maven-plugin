...
public class ScalaDocJarMojo extends ScalaDocMojo {
  ...
  /**
   * Specifies the filename that will be used for the generated jar file. Please note that <code>
   * -javadoc</code> or <code>-test-javadoc</code> will be appended to the file name.
   */
  ...
  private File generateArchive(File javadocFiles, String jarFileName)
    ...
    if (useDefaultManifestFile
        && defaultManifestFile.exists()
        && archive.getManifestFile() == null) {
      getLog()
          .info(
              "Adding existing MANIFEST to archive. Found under: " + defaultManifestFile.getPath());
